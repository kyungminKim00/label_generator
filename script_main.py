...
def update_graph_live(n, n_clicks, n_clicks_state):
    ...
    data = [
        ...
        go.Scatter(x=df['Date'][:n], y=df['10_day_MA'][:n], mode='lines', name='10일 이동평균'),
        go.Scatter(x=df['Date'][:n], y=df['50_day_MA'][:n], mode='lines', name='50일 이동평균'),
        go.Scatter(x=df['Date'][:n], y=df['100_day_MA'][:n], mode='lines', name='100일 이동평균')
    ]
    ...
    if n_clicks is not None and n_clicks > 0 and n_clicks_state > 0:
        ...
        h_line = {
            ...
            'yref': 'y', 'y0': line_level, 'y1': line_level,
            'line': {'color': 'black', 'width': 1, 'dash': 'dash'}
        }
        v_line = {
            'type': 'line',
            'xref': 'x', 'x0': df['Date'][n-1], 'x1': df['Date'][n-1],
            'yref': 'paper', 'y0': 0, 'y1': 1,
            'line': {'color': 'black', 'width': 1, 'dash': 'dash'}
        }
        layout = go.Layout(xaxis = {'rangeslider': {'visible': False}}, shapes=[h_line, v_line])
    else:
        layout = go.Layout(xaxis = {'rangeslider': {'visible': False}})
    
    return {'data': data, 'layout': layout}

@app.callback(Output('mark-price-button', 'n_clicks'),
              [Input('interval-component', 'n_intervals')]
              )
